...
int block_allocator_init(
    ...
    allocator->free_list_head = (block_header_t *)allocator->virtual_base;

    block_header_t *block_header = (block_header_t *)allocator->virtual_base;
...
static block_header_t *find_or_create_free_block(block_allocator_t *allocator, uint64_t size) {
    ...
    uint64_t num_pages = (size + sizeof(block_header_t)) / MAP_PAGE_SIZE;
    if ((size + sizeof(block_header_t)) % MAP_PAGE_SIZE != 0) {
        num_pages++;
    }
    ...
    new_block_header->size = num_pages * MAP_PAGE_SIZE - sizeof(block_header_t);
